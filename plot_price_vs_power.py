                    ...
                    for trimname in year.keys():
                        ...
                        if 'price' in trim.keys():
                            ...
                            if 'horsepower' in enginedict.keys():
                                ...
                                if 'size' in enginedict.keys():
                                    displacement = enginedict['size']
                                    hpperliter = power/displacement
                                    carname = (yr + ' ' + makename + ' ' + modelname + ' ' + trimname)
                                    point, = pyplot.plot(price, hpperliter, 'ro', markersize=5, alpha=0.5)
                                    annotation = ax.annotate((carname + '\n$'+ str(price)+'0\n'+str(hpperliter)+' hp/L') , (price,hpperliter), xycoords='data', xytext=(price,hpperliter), textcoords='data')
